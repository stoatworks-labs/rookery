...
| Platform | Directory |
| --- | --- |
| macOS | `~/Library/Logs/rookery/` |
| Linux | `$XDG_STATE_HOME/rookery/logs/` (default `~/.local/state/...`) |
| Windows | `%LOCALAPPDATA%\rookery\logs\` |

`ROOKERY_LOG_DIR` overrides it. The path is printed on the first line of every
...
`rookery.YYYY-MM-DD.log`, rotated daily, seven kept, no colour escapes.

Verbosity comes from `ROOKERY_LOG` (falling back to `RUST_LOG`), default `rookery=info`.
...
`rookery-crash-<timestamp>.json`; the path is printed on the way down. The
...
```bash
rookery --collect-diagnostics
```

Writes `rookery-diagnostics-<timestamp>.json` and prints its path. One file, so
